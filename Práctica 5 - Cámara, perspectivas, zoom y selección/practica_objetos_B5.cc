	...
	switch (toupper(Tecla1)){
		...
		case '6':modo=SOLID_ILLUMINATED_GOURAUD;break; // Práctica 4
	        ...
	        case 'R':t_objeto=ROTACION;break;
					...
					case 'Z':cambio=0;break; // Práctica 5
					case 'X':cambio=1;break; // Práctica 5
					case '+':factor=factor*0.9;break; // Práctica 5
					case '-':factor=factor*1.1;break; // Práctica 5
		}
	glutPostRedisplay();
}
...
void procesar_color(unsigned char color[3]){
	...
	for(i=0; i<mago.piezas; i++){
		if(color[0]==mago.color_selec[0][i]){
			if(mago.activo[i]==0){
				mago.activo[i]=1;
			}else{
				mago.activo[i]=0;
...
void initialize(void){
	...
	glClearColor(0.439216,0.576471,0.858824,1);
